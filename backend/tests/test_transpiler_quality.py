...
def test_prepare_java_renames_public_main_class_for_judge0():
    source = """
class CodeConverter {
...
public class CodeConverterTest {
    public static void main(String[] args) {
        System.out.println(CodeConverter.celsiusToFahrenheit(0));
    }
}
"""
    prepared = prepare_java_for_judge0(source)
    assert "public class Main" in prepared
    assert "public class CodeConverterTest" not in prepared
    assert "class CodeConverter" in prepared
    assert "CodeConverter.celsiusToFahrenheit" in prepared


def test_prepare_java_leaves_existing_main_class():
    source = "public class Main { public static void main(String[] args) { System.out.println(1); } }"
    prepared = prepare_java_for_judge0(source)
    assert prepared.count("public class Main") == 1
